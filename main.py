...
🔔️  <b>/new</b> - создать напоминание
🔈️  <b>/current</b> - текущие напоминания
🔇️  <b>/done</b> - выполненные напоминания 
✏️  <b>/edit</b> - изменить напоминание
🗑️  <b>/delete</b> - удалить напоминание
🗂️  <b>/exit</b> - выйти в главное меню
"""
...
async def notification_function():
    print("зашел в notification_function")
...
@dp.message_handler(text=['Текущие дела', '/current'])
...
async def current_tasks(message:types.Message):
    ...
    for task in tasks:
        ...
        not_done_tasks += f"<b>✅  {task[3]}</b> - <b>{task[1]}</b>\n {task[2]}\n"
        num = num + 1
...
async def delete_tasks_by_name(message: Message):
    await bot.send_message(chat_id=message.from_user.id,
                       text='Выберите напоминание, который хотите удалить',
...
async def del_current_task(callbak:CallbackQuery, state:FSMContext):
    ...
    await bot.send_message(chat_id=callbak.from_user.id, text = '✅  Напоминание успешно удалено!')
    await state.finish()
...
#запланировать дело
@dp.message_handler(text=['Создать напоминание', '/new'])
...
async def call_calendar(message: Message, state: FSMContext):
    ...
    await message.answer("Выберите дату напоминания: ", reply_markup=await SimpleCalendar().start_calendar())
...
async def process_simple_calendar(callback_query: CallbackQuery, callback_data: dict,state: FSMContext):
    ...
    if selected:
        await callback_query.message.answer(
            f'Дата напоминания  {date.strftime("%d/%m/%Y")}',
...
async def edit_projects(message: types.Message):
    await bot.send_message(chat_id=message.from_user.id,
                           text='Выберите напоминание, который хотите редактировать',
...
async def upload_files(callback:CallbackQuery):
    await bot.send_message(chat_id=callback.from_user.id,
                           text='Введите название файла... В формате <strong>FileName</strong>.формат(txt,xlsx,...) ',
                           parse_mode='HTML')
...
async def edit_periodic_state(callback:CallbackQuery, state:FSMContext):
    ...
    print(period)
    if period[0] == 0:
        await bot.send_message(chat_id=callback.from_user.id, text='Введите периодиодичность напоминания (в днях)')
        await EditTasks.Edit_Periodic.set()
    else:
        await update_pereodic_of_task_no(callback.from_user.id, state)
        await bot.send_message(chat_id=callback.from_user.id, text = '✅  Успешно! Напоминание теперь не периодическое')
        await state.finish()
...
async def set_period(message:Message,state:FSMContext):
    ...
    await message.answer(f'✅  Успешно! Периодичность напоминания = {message.text}')
    await state.finish()
...
async def edit_done_station(callback: CallbackQuery,state: FSMContext)->None:
    ...
    if done == 0:
        ...
        await bot.send_message(chat_id=callback.from_user.id,
                               text='✅  Успешно! Дело возвращено в список текущих. Необходимо заново установить дату и время для напоминания.')
        flag = 0
        await edit_state_of_task(state, callback.from_user.id,flag)
    await EditTasks.Get_inline_menu.set()
